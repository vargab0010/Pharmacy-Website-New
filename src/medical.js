...
window.onload = async () => {
  try {
    const res = await fetch('http://localhost:5000/api/products/category/medical');
    const products = await res.json();

    const groups = {
      kids: document.getElementById('kids-products'),
      adults: document.getElementById('adults-products'),
      seniors: document.getElementById('seniors-products'),
      general: document.getElementById('general-products')
    };
    ...
    products.forEach(p => {
      ...
      const groupContainer = groups[p.ageGroup?.toLowerCase()];
      if (groupContainer) {
        groupContainer.innerHTML += productHTML;
      }
    });
  } catch (err) {
